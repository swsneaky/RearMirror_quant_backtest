...
def test_incremental_ingest_queues_rebuild_when_factors_change(tmp_path):
    cfg = {
        "database": {"path": str(tmp_path / "quant.db")},
        "etl": {"industry_map": str(tmp_path / "missing_industry.parquet")},
    }
    ...
    revised = seed.copy()
    revised["fwd_factor"] = [0.95]

    try:
        _ingest_to_db(seed, cfg, update_mode="full")
        _ingest_to_db(revised, cfg, update_mode="incremental")

        con = get_connection(cfg)
        row = con.execute(
            "SELECT COUNT(*) FROM factor_rebuild_queue WHERE code = ? AND status = 'pending'",
            ["sh.600000"],
        ).fetchone()
        assert row[0] == 1
    finally:
