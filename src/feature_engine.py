...
def _get_required_raw_cols(name: str) -> list[str]:
    ...
    if meta is not None:
        # meta.input_cols 是逻辑列名, 需补充 cum_factor (复权需要)
        cols = list(meta.input_cols)
        if any(c in cols for c in ["raw_close", "raw_high", "raw_low", "raw_open"]):
            if "cum_factor" not in cols:
                cols.append("cum_factor")
        return cols

    # legacy fallback
    _legacy = {
        'kline': ["raw_open", "raw_close", "raw_high", "raw_low", "raw_volume", "cum_factor"],
        'rolling': ["raw_close", "raw_high", "raw_low", "raw_volume", "raw_pctChg", "cum_factor"],
        'rolling_ext': ["raw_close", "raw_high", "raw_low", "raw_volume", "raw_pctChg", "cum_factor"],
        'technical': ["raw_close", "raw_high", "raw_low", "raw_volume", "raw_amount", "raw_pctChg", "cum_factor"],
        'turnover': ["raw_turn"],
...
def build_alpha158(
    ...
    **断点续传**: 全量计算时, 每完成一个因子组 checkpoint 到磁盘;
    ...
    # 复权价 (内部计算用辅助列, 以 _ 开头)
    df["_close_adj"] = df["raw_close"] * df["cum_factor"]
    df["_open_adj"] = df["raw_open"] * df["cum_factor"]
    df["_high_adj"] = df["raw_high"] * df["cum_factor"]
    df["_low_adj"] = df["raw_low"] * df["cum_factor"]
    ...
    print(f"[2/2] 启动因子流水线, {mode_tag}计算 {len(target_groups)} 个因子组: {target_groups}")
    n_rows = len(df)
