...
def run_raw_feature_pipeline(
    ...
    def _run_db_first_increment(
        ...
        if n_new_dates == 0:
            ...
            _progress(100, "No new daily_bar dates; reused SQLite feature store")
            return raw_df, all_features, group_feature_map
        ...
        try:
            def incr_progress_cb(pct: int, msg: str):
                if progress_cb:
                    progress_cb(10 + int(pct * 0.85), msg)

            computed_df, computed_features, computed_group_map = build_alpha158(
                cfg,
        ...
        raw_new_rows = _select_raw_feature_cols(new_rows, computed_features)
        stats = feature_store.upsert_raw_features(
            raw_new_rows,
            computed_features,
            extra_cols=raw_extra_cols,
        ...
        raw_df = feature_store.load()
        _record_raw_feature_fingerprint(raw_df, computed_features, source="db_first_incremental")

        print(
            "[INCR] raw feature DB-first stats: "
            f"old_feature_max={old_max.date()} daily_bar_max={daily_max.date()} "
            f"new_trading_dates={n_new_dates} warmup_start={warmup_start.date()} "
            f"computed_rows={len(computed_df):,} upserted_rows={stats['upserted_rows']:,} "
        ...
        try:
            ...
            if existing_features:
                ...
                try:
                    ...
                    is_fresh, reason = check_cache_fresh(raw_output, cfg, sections=["etl", "features"])
                    if not is_fresh:
    ...
    try:
        from src.storage_manager import save_fingerprint
        save_fingerprint(
            raw_output, cfg, sections=["etl", "features"],
...
def run_neutralize_pipeline(
    ...
    try:
        from src.storage_manager import save_fingerprint
        save_fingerprint(
            out, cfg, sections=["etl", "features", "cross_section", "label"],
            row_count=len(df),
...
def run_label_pipeline(cfg: dict | None = None, incremental: bool = True) -> dict:
    ...
    sql = f"""
        SELECT db.date, db.code, db.raw_close, db.raw_pctChg, db.industry
        FROM daily_bar db
    ...
    elif label_method == "close_ratio":
        # 未来 horizon 天的收益率 (close ratio)
