...
## [20260428_shared_machine_canonical_entry_issues]
- status: open
- axis: governance_and_operationalization
- stage_or_slice: runtime_modes_and_degradation
- related_business_stage: train_and_backtest
- summary: run_backtest_pipeline(runtime_mode="shared_machine") canonical 入口因 2 个代码问题无法直接运行。(1) pipeline.py L1010 -> build_train_dataset() 默认 max_missing_ratio=0.0 对 shared_machine reduced date range 过严（feature_only ~2.0%）；(2) src/ml_core/backtest.py L317 model.fit() 未提供 eval_set，但 configs/base_config.yaml L169 配置 early_stopping_rounds: 50。
- why_not_blocking_now: Session D 已通过 workaround 证明底层 pipeline 逻辑正确（3 个 Stage 全部 PASS）。2 个问题为局部代码修复项，不影响"pipeline 链路逻辑正确"的核心验证结论。
- required_next_owner: Session B
- required_next_action: (1) 为 build_train_dataset() 在 shared_machine 模式下支持 skip_pair_validation 或按 runtime_mode 动态调整 max_missing_ratio；(2) 为 WFA fit() 提供 eval_set 或在无 eval_set 时自动禁用 early_stopping_rounds。修复后交 Session D 验证 canonical 入口可直接运行。
- evidence: Session C 审计报告 (WORKLOG.md 2026-04-28)；代码点 dataset_builder.py L92/L166、backtest.py L317、base_config.yaml L169；验证脚本 tools/session_d_verify_staged.py
